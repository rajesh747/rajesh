...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=20 --timeout-seconds=20 -n ${GUID}-tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=20 --timeout-seconds=20 -n ${GUID}-tasks-prod
...
oc set resources dc tasks-blue --limits=memory=4Gi,cpu=4 --requests=memory=3Gi,cpu=2 -n ${GUID}-tasks-prod
...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=20 --timeout-seconds=20 -n ${GUID}-tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=20 --timeout-seconds=20 -n ${GUID}-tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
